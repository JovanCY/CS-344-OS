...
{ // call syntax: enc_client plaintext key port
    ...
    char key[70000];
    char message[70000];
    char messageLength[10]; // message length to send to server
    char accMsg[20];        // to hold message length response from server
    int messageSize;
    ...
    for (int i = 0; i < strlen(buffer); i++)
    {
        if ((int)buffer[i] < 65)
        {
            ...
            {
                // printf("int buffer = %d at location : %d\n", (int)(buffer[i]), i);
                error("CLIENT: the plaintext has bad characters!");
            }
        }
        else if ((int)buffer[i] > 91)
        {
    ...
    charsRead = recv(socketFD, buffer, atoi(messageLength), 0);
    if (charsRead < 0)
    {
        error("CLIENT: ERROR reading from socket");
    }
